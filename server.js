...
async function refreshAzjaFlightsRoundtrip() {
    ...
    for (const from of searchFromAirports) {
        for (const dest of azjaAirports) {
            for (const monthOutbound of months) {
                for (const monthInbound of months) {
    ...
    for (const key in azjaFlightsCache) {
        azjaFlightsCache[key] = azjaFlightsCache[key].map(flight => {
...
// Uruchomienie pierwszego odświeżenia przy starcie serwera,
// kolejne będzie wykonywane przez Twój cron na Renderze.
